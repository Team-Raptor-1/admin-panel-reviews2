...
const Reviews = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await Axios.get("http://localhost:3002/api/rretrieve");
    ...
    const handleDelete = async (id) => {
      try {
        await Axios.delete(`http://localhost:3002/api/rdelete/${id}`).then(
          () => {
    ...
    const handleretrieve = async (id) => {
      try {
        await Axios.get(`http://localhost:3002/api/rretrieve/${id}`).then(
          () => {
            window.location.reload();
          }
        );
    ...
    const handlepost = async (id) => {
      try {
        await Axios.post(`http://localhost:3002/api/rpost/${id}`).then(() => {
          window.location.reload();
        });
        console.log("Data deleted successfully");
      } catch (error) {
